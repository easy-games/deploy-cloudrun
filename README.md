...
Use the flag `setup_telemetry` to setup a default prometheus monitoring sidecar container for the service, default is false.
...
-   `env_vars`, `env_vars_file`, and `env_vars_update_strategy`: (Optional)
    ...
    authoritative). The default value is "overwrite".
